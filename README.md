...
## Альтернативы kill switch

Kill switch в этом сервисе реагирует примерно за **1 секунду** (интерфейс сканируется каждую секунду). За это время короткий запрос теоретически может уйти напрямую. Если нужна 100% защита, есть два более надёжных подхода:

**Network namespace** — создать отдельный сетевой namespace и перенести в него только VPN-интерфейс. Приложения (Codex, браузер) запускаются внутри namespace. Если VPN падает — у namespace просто нет интернета. Железная изоляция, но сложнее в настройке (особенно с sing-box и GUI-приложениями).

**Локальный прокси** — запустить прокси, привязанный к IP VPN-интерфейса. Приложения ходят через `HTTP_PROXY=http://127.0.0.1:8080`. Если VPN не работает — прокси не слушает, приложения получают connection refused. Просто, но не все программы уважают `HTTP_PROXY`, и возможны DNS-утечки.

Текущий индикатор + kill switch — баланс между удобством и защитой. Для большинства сценариев достаточно.
