...
Kill switch в этом сервисе реагирует примерно за **1 секунду**. За это время короткий запрос теоретически может уйти напрямую. Если хочется дополнительную защиту, есть два варианта:

### Network namespace
Создать отдельный сетевой namespace и перенести в него только VPN-интерфейс. Приложения запускаются внутри namespace — если VPN падает, у них просто нет интернета. Надёжно, но сложнее в настройке.

### Локальный прокси через sing-box
Если sing-box уже запущен, проще всего добавить в его конфиг socks5/http inbound и пускать Codex через него. Если sing-box упал — прокси перестаёт работать и Codex не соединится.

**Настройка:**

1. Добавь в `config.json` sing-box inbound (рядом с `tun`):
```json
{
  "type": "socks",
  "listen": "127.0.0.1",
...
2. Установи `proxychains-ng`:
```bash
sudo apt install proxychains4
```

3. В `/etc/proxychains4.conf` в конце файла замени `socks4 127.0.0.1 9050` на:
```
socks5 127.0.0.1 1080
```

4. Запускай Codex так:
```bash
proxychains4 codex
```

Теперь весь трафик Codex идёт через sing-box. Если sing-box выключится — Codex сразу потеряет связь.

> **Примечание:** proxychains перехватывает соединения на уровне сети (через LD_PRELOAD), поэтому работает даже если программа не уважает `HTTP_PROXY`.
