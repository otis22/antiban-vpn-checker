...
`install.sh` уже настраивает автозапуск через systemd user service + `.desktop` файл. При входе в графическую сессию приложение стартует само.

Проверь статус:
```bash
systemctl --user status vpn-checker.service
```

Если после ребута не стартует — включи **linger** (чтобы user services жили между сессиями):
```bash
sudo loginctl enable-linger $USER
```

> Tray-иконка появляется только после входа в графическую сессию. До логина процесс работает в фоне без GUI.
